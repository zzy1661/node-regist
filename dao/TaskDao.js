...
class TaskDao {
    ...
    addTask(task) {
        return new Promise((resolve, reject) => {
            var inputs = ['name', 'des', 'pid', 'startime', 'endtime', 'creatime', 'status', 'uid', 'del'];
    ...
    updateTask(task) {
        return new Promise((resolve, reject) => {
            var inputs = ['name', 'des', 'pid', 'endtime', 'status','del'];
            var updateFields = [];
            var values = [];
            inputs.forEach(i => {
    ...
    queryTasks(task) {
        return new Promise((resolve, reject) => {
            ...
            if (task) {
                ...
                if (startime !== null && startime !== undefined) {
                    conditions.push('startime = ?');
                    values.push(startime)
                }
                ...
                if (creatime !== null && creatime !== undefined) {
                    conditions.push('creatime = ?');
                    values.push(creatime)
                }
                conditions.push('uid = ?');
                values.push(task.uid)
            }
            var conditionStr = conditions.length > 0 ? `where ${conditions.join(' and ')}` : '';
            var sql = `select id,name,des,pid,startime,endtime,status,creatime,uid from tasks ${conditionStr} and del = 0`;
